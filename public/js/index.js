...
var amount, description, type, tag, source, date, file;
var name, note;
...
// var firebase;
var firebaseDB, spendRef, earningRef, tipRef, todoRef, allRef, fileRef;
var storageRef;
window.onload = function () {
    document.getElementById("add_expense_submit_button").onclick = add_expenses;
    ...
    spendRef = firebaseDB.ref('spends');
    earningRef = firebaseDB.ref('earnings');
    allRef = firebaseDB.ref('expense-6ad16');
    // storage ref
...
async function create_table(path, eleId) {
    let list = []
    var ref = firebaseDB.ref(path);
    (function () {
...
function list_expenses() {
    var spends = create_table('spends/','expenses_display');
    var earnings = create_table('earnings/','earnings_display');
...
function authenticate() {
    ...
    //using jquery
    var Url = 'https://us-central1-expense-6ad16.cloudfunctions.net/verifyPassword?password=' + password;
    $.ajax({
        url: Url,
        type: "GET",
...
function delete_note() {
    ...
    userDetails = "Name : " + name + "<br />description : " + description +
...
function fetch_notes() {
    ...
    var descriptionField = document.getElementById('search_description');
    ...
    if (description) userDetails += "description : " + description + "<br />";
    ...
    function formValidation() {
        ...
        if (!(nameField.value || descriptionField.value || tagField.value || keywordField.value || startDateField.value || endDateField.value || typeField.value)) {
...
function add_expenses() {
    ...
    var amountField = document.getElementById('add_amount');
    var descriptionField = document.getElementById('add_description');
    var typeField = document.getElementById('add_type_selector');
    var sourceField = document.getElementById('add_source_selector');
    ...
    description = descriptionField.value;
    tag = tagField.value;
    source = sourceField.options[sourceField.selectedIndex].value;
    date = dateField.value;
    type = typeField.options[typeField.selectedIndex].value;

    console.log("assigned values");

    userDetails = "amount : " + amount + "<br />description : " + description + "<br />Type : " + type +
    ...
    function formValidation() {
        ...
        if (!(amountField.value)) {
            showAlert("add_expense_alert", "alert-danger", null, "Form Validation Failed", null, "Name cannot be null");
            return false;
        }
        if (!(descriptionField.value)) {
            showAlert("add_expense_alert", "alert-danger", null, "Form Validation Failed", null, "description cannot be null");
            return false;
        }
        if (!(typeField.value)) {
            showAlert("add_expense_alert", "alert-danger", null, "Form Validation Failed", null, "type cannot be null");
            return false;
        }
        if (!(dateField.value)) {
            showAlert("add_expense_alert", "alert-danger", null, "Form Validation Failed", null, "Date cannot be null");
            return false;
        }
        return userDetails;
    }
    ...
    if (Validation) {
        firebaseSaveData();
        showAlert("add_expense_alert", "alert-success", null, "Expense Added!", null, userDetails);
    }
...
function update_note(key) {
    ...
    function formValidation() {
        ...
        // console.log("name : " + nameField.value);
        if (!(descriptionField.value)) {
            showAlert("edit_note_alert", "alert-danger", null, "Form Validation Failed", null, "description can not be null");
            return false;
        }
        // console.log("descriptionField : " + descriptionField.value);
        if (!(tagField.value)) {
...
function showAlert(parent, alertClassName, headingClassName = "alert-heading", alertHeading, textClassName = "mb-0", alertText) {
    ...
    $(".alert").click(function () {
        ...
        $(".alert").alert('close')
        // clearChilds("expense_display");
        var container = document.getElementById("expense_display");
...
function filterData() {
    ...
    function addToNotesPriorityList(i) {
        if (name && noteData[i].name.toUpperCase() === name.toUpperCase()) {
            ...
            preciselyFilteredData.push(noteData[i])
            preciselyFilteredKey.push(noteKey[i])
        } else if (description && noteData[i].description.toUpperCase() === description.toUpperCase()) {
            preciselyFilteredData.push(noteData[i])
            preciselyFilteredKey.push(noteKey[i])
            ...
            filteredData.push(noteData[i])
            filteredKey.push(noteKey[i])
        } else if (description && noteData[i].description.toUpperCase().includes(description.toUpperCase())) {
            filteredData.push(noteData[i])
            filteredKey.push(noteKey[i])
            ...
            if (!(name || keyword || description || tag)) {
                filteredData.push(noteData[i])
                filteredKey.push(noteKey[i])
            }
        }
...
function addExpenseToDisplay(eleId, expense, count) {
    console.log("Adding elements to display",expense)
...
//     innerDiv.setAttribute("data-parent", "#expense_display");
//     card.appendChild(innerDiv);


//     var innerDiv2 = document.createElement("div");
//     innerDiv2.setAttribute("class", "card-body");
//         innerDiv2.innerHTML = "<pre><b>Description  : " + expense.description + "</b></pre>"
//                             + "<pre><b>Amount       : " + expense.amount + "</b></pre>"
//                             + "<pre><b>Type         : " + expense.type + "</b></pre>"
//                             + "<pre><b>Source       : " + expense.source + "</b></pre>"
//                             + "<pre><b>Date         : " + expense.date + "</b></pre>"
//     innerDiv.appendChild(innerDiv2);
...
function firebaseSaveData() {
    ...
    (function () {
        ...
        switch (type) {
            case "spend":
                console.log("SpendRef", spendRef)
                spendRef.push({ amount: amount, description: description, tag: tag, source: source, date: date });
                break;
            case "earning":
                earningRef.push({ amount: amount, description: description, tag: tag,source: source, date: date });
                break;
            default:
                alert("No type found");
        }
...
function firebaseUpdateData(key, userDetails) {
    ...
    try {
        (function () {
            ...
            switch (type) {
                case "spend":
                    spendRef.child(key).update({ name: name, description: description, tag: tag});
                    break;
                case "earning":
                    earningRef.child(key).update({ name: name, description: description, tag: tag});
                    break;
                default:
                    alert("No type found");
            }
